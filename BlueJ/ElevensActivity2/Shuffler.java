...
public class Shuffler {
    ...
    public static int[] perfectShuffle(int[] values) {
        ...
        if (values.length % 2 == 0) {
            length1 = values.length/2;
        } else {
            length1 = (values.length + 1)/2;
          }
        ...
        k = 1;
        for (int j = length1; j < values.length; j++) {
          shuffled[k] = values[j];
          k += 2;
        }
        return shuffled;
     ...
     * car  ds (initially the entire deck). It repeatedly does the following until
